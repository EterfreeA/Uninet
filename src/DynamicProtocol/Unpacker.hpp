...
public:
	...
	public:
		...
		explicit Table(std::string&& _stream) noexcept
			: _stream(std::move(_stream))
		{
			parse();
		}

		Table(const char* _data, size_t _size)
			: _stream(_data, _size)
		{
			parse();
		}
		...
		Table& operator=(std::string&& _stream)
		{
		...
		Table& set(const char* _data, size_t _size)
		{
		...
		bool getTable(Number _number, Table& _table);
...
public:
	explicit Unpacker(const std::string& _stream) : _stream(_stream) {}
	explicit Unpacker(std::string&& _stream) noexcept : _stream(std::move(_stream)) {}

	Unpacker& operator=(const std::string& _stream)
	{
		this->_stream = _stream;
		return *this;
	}

	Unpacker& operator=(std::string&& _stream)
	{
		this->_stream = std::move(_stream);
		return *this;
	}

	Table& unpack(Table& _table)
	{
		return _table = _stream;
	}
};

template <typename _Source, typename _Target>
std::optional<_Target> Unpacker::Table::get(Number _number, TYPE _type)
{
...
template <typename _Source, typename _Target>
bool Unpacker::Table::getVector(Number _number, TYPE _type, std::vector<_Target>& _vector)
{
	auto iterator = _fields.find(_number);
	if (iterator == _fields.end())
		return false;
