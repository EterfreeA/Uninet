...
* 
* 摘要：
* 1.原型为有限状态机，不过可以自由组合层次状态机，提供添加、移除、查找指定状态等方法。
* 2.支持序列化与反序列化，采用深度优先遍历方式。序列化用于收集与释放所有状态，反序列化用于分发状态以构造状态机。
* 3.支持备份与还原状态树的有效节点路径，即有效状态编号向量。
* 4.支持状态访问所属状态机，实现侧向或者向上状态转换。另支持按照深度优先顺序连续转换状态。
* 5.输入驱动是状态机的核心思想。输入事件的处理方法与过渡实例结合，支持在转换状态之前，动态生成目标状态。
* 
* 变化：
* v1.0.1
* 1.在添加状态之时，判断是否添加自身，避免循环引用。
*/
...
public:
	...
	typedef State<_IDType, _MessageType, _SizeType> StateType;
...
private:
	 ...
	 *     1.过渡实例：指示转换方式。
	 * 返回值：
	 *     在转换状态之后，倘若不能够再次转换，返回false；若仍然存在转换的可能性，则返回true。
	 */
	bool update(TransitionType& _transition)
	{
		...
		// 若目标状态不存在，说明状态机出现异常，可能是目标状态错误，或者未提前添加目标状态
...
public:
	...
	virtual TransitionType handle(MessageType _message)
	{
		...
		// 根据过渡实例转换状态，在转换状态之后，若不能够再次转换，返回过渡实例
	 ...
	 *
	 * 参数：
	 *     1.节点向量：反序列化状态节点，按照深度优先构造状态树，起始位置应是空节点，且指定子节点数量。
	 *     2.游标：索引当前状态节点，初始需要指向空节点。
	 * 返回值：
	 *     构造失败返回false，构造成功返回true。
	 *
	 * 若游标大于等于节点数量，则参数有误，构造失败，可能是节点数量或者顺序错误。
	 * 若添加节点失败，则构造失败，可能是添加相同编号的节点至同一位置。
	 */
	 ...
	 *
	 * 若未找到指定编号的节点，则还原失败，可能是有效状态编号向量或者游标初始位置有误，也可能是存在动态节点，而未提前生成节点。
	 */
	 ...
	 * 若目标状态是当前状态或者已经过状态，则不会转换状态。
	 * 若目标状态不存在，则转换直至最后状态退出，并且状态机设为无效类型。
	 * 
	 * 前提条件：
	 *     状态在整体上呈线性分布。
	 * 参数：
	 ...
	 *     转换失败返回false，转换成功返回true。
	 * 
	 * 若游标大于等于节点数量，则参数有误，转换失败，可能是节点数量或者顺序错误。
	 ...
	 *     1.状态编号：用于索引添加的状态。
	 *     2.状态指针：指向已创建的状态实例。
	 *     3.状态编号向量：按照自上而下的层级顺序搜索状态机。
	 *     4.游标：索引当前状态机。
	 * 返回值：
	 *     添加失败返回false，添加成功返回true。
	 * 
	 * 若层级大于状态编号数量，则添加失败，可能是状态编号向量或者初始层级有误。
	 * 若未找到指定状态机，则添加失败。
	 */
	 ...
	 * 
	 * 若当前状态机有效，并且待移除状态是当前状态，或者未找到待移除状态，则移除失败。
	 */
	 ...
	 * 参数：
	 *     1.状态编号：索引待移除状态。
	 *     2.状态编号向量：按照由上至下的层级顺序搜索状态。
	 *     3.层级：索引当前状态。
	 * 返回值：
	 *     移除失败返回NULL，移除成功返回状态指针。
	 * 
	 * 若层级大于状态编号数量，则移除失败，可能是状态编号向量或者初始层级有误。
	 * 若未找到目标状态，则移除失败。
	 */
	 ...
	 * 参数：
	 *     1.状态编号：索引目标状态。
	 *     2.状态编号向量：按照自上而下的层级顺序搜索状态。
	 *     3.层级：索引当前状态。
	 * 返回值：
	 *     设置失败返回false，设置成功返回true。
	 * 
	 * 若层级大于状态编号数量，则设置失败，可能是状态编号向量或者初始层级有误。
	 * 若未找到目标状态，则设置失败。
	 */
	 ...
	 *     获取失败返回NULL，获取成功返回状态指针。
	 * 
	 * 若层级大于状态编号数量，则获取失败，可能是状态编号向量或者初始层级有误。
	 * 若未找到目标状态，则获取失败。
	 */
